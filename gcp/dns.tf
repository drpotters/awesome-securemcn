...
# A better way to do this is just add a forwarder to CoreDNS, assuming there will only GKE clusters here.
/*
locals {
...
resource "google_dns_managed_zone" "inside" {
  for_each    = var.business_units
  project     = var.gcpProjectId
  name        = format("%s-private-%s-%s", var.projectPrefix, each.key, local.buildSuffix)
  dns_name    = format("%s.", var.domain_name)
  description = format("Resolve all %s hosts to Volterra gateway (%s-%s)", each.key, var.projectPrefix, local.buildSuffix)
