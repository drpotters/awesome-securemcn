...
locals {
  ...
  name            = coalesce(var.name, format("%s-wkstn-%s-%s", var.projectPrefix, random_id.nonce.hex, local.buildSuffix))
  zone            = coalesce(var.zone, random_shuffle.zones.result[0])
  service_account = coalesce(var.service_account, format("%s-wkstn-%s@%s.iam.gserviceaccount.com", var.projectPrefix, local.buildSuffix, var.gcpProjectId))
  tls_secret_key  = var.tls_secret_key != "" ? var.tls_secret_key : null
}
...
module "sa" {
  ...
  names        = [format("wkstn-%s", local.buildSuffix)]
  descriptions = [format("Workstation for %s (%s)", var.projectPrefix, local.buildSuffix)]
  project_roles = [
...
module "tls_secret" {
  count                   = local.tls_secret_key
  source                  = "../tls/"
  gcpProjectId            = var.gcpProjectId
  secret_manager_key_name = format("%s-wkstn-tls-%s", var.projectPrefix, local.buildSuffix)
  secret_accessors = [
...
resource "google_compute_firewall" "iap" {
  project = var.gcpProjectId
  name    = format("%s-iap-wkstn-%s-%s", var.projectPrefix, random_id.nonce.hex, local.buildSuffix)
