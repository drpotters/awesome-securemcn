locals {
  ...
  aws_region = data.tfe_outputs.root.values.awsRegion
  ...
  kubeconfig_name = format("%s-%s-aws-kubeconfig", local.projectPrefix, data.tfe_outputs.root.values.buildSuffix)
  ...
  aws_service_name = format("%s-nic-%s-nginx-ingress-controller", local.projectPrefix, local.buildSuffix)
  aws_service_endpoint_ip = join("", flatten(data.kubernetes_endpoints_v1.origin-pool-k8s-service.subset[*].address[*].ip))
  app_domain = data.tfe_outputs.root.values.app_domain
